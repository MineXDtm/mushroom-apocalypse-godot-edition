...
func _on_buy3_pressed():
	for item in PlayerInventory.inventory1:
		if PlayerInventory.inventory1[item][0] == "drop_wood" and PlayerInventory.inventory1[item][1] >= 1:
			print("crafted")
			...
			PlayerInventory.add_item("block_beta", 1)
			return
	for item in PlayerInventory.inventory:
		if PlayerInventory.inventory[item][0] == "drop_wood" and PlayerInventory.inventory[item][1] >= 1:
			print("craf2ed")
