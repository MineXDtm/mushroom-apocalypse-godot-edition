...
func _on_buy_pressed():
	for item in PlayerInventory.inventory1:
		...
		for item2 in PlayerInventory.inventory1:
			...
			if PlayerInventory.inventory1[item][0] == "drop_wood" and PlayerInventory.inventory1[item][1] >= 1 and PlayerInventory.inventory1[item2][0] == "cobblestone" and PlayerInventory.inventory1[item2][1] >= 3:
				PlayerInventory.inventory1[item][1] -= 1
				PlayerInventory.inventory1[item2][1] -= 3
				...
				PlayerInventory.add_item("stone_exe", 1)
				return
				...
				for item3 in PlayerInventory.inventory:
					if PlayerInventory.inventory[item3][0] == "cobblestone" and PlayerInventory.inventory[item3][1] >= 3:
						PlayerInventory.inventory1[item][1] -= 1
						PlayerInventory.inventory[item3][1] -= 3
						if PlayerInventory.inventory1[item][1] == 0:
							PlayerInventory.inventory1.erase(item)
							var inventory1 = get_parent().get_parent().get_parent().get_parent().get_parent().get_parent().get_node("inventory")
							inventory1.finder(item)
	...
	for item in PlayerInventory.inventory:
		for item2 in PlayerInventory.inventory:
			if PlayerInventory.inventory[item][0] == "drop_wood" and PlayerInventory.inventory[item][1] >= 1 and PlayerInventory.inventory[item2][0] == "cobblestone" and PlayerInventory.inventory[item2][1] >= 3:
				PlayerInventory.inventory[item][1] -= 1
				PlayerInventory.inventory[item2][1] -= 3
