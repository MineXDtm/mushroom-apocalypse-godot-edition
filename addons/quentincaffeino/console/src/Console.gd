...
# @param  InputEvent  e
func _input(e):
	if Input.is_action_just_pressed("console") and OS.is_debug_build():
		self.toggle_console()
...
func toggle_console():
	...
	var p = get_tree().get_root().get_node_or_null("world/map/sort/player")
	if p != null:
		p.in_console = is_console_shown
	return self
