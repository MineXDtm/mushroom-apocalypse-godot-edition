...
var categories = [instrements,farm,builds,drops,mechanisms]
var categories_names = ["instruments","farm","builds","drops","mechanisms"]
func _ready():
	var button = load("res://gui/menubut.tscn")
	for c in int(categories.size()):
		...
		for i in category:
			...
			if category[i].size() == 3:
				...
				button_instance.get_node("buy1").nedded2 = category[i][1]
				button_instance.get_node("buy1").needed_number2 = category[i][2]
				button_instance.get_node("buy1").done = i
				button_instance.get_node("buy1").done_number = category[i][0]
				if category[i][2] == 1:
				...
				button_instance.get_node("buy1").done = i
				button_instance.get_node("buy1").done_number = category[i][0]
				button_instance.get_node("buy1").nedded1 = category[i][3]
				button_instance.get_node("buy1").needed_number1 = category[i][4]
				button_instance.get_node("need2/Label").text =  str(category[i][2])
				button_instance.get_node("need2").texture = load("res://textures/items/" + category[i][1] + ".png")
				if category[i][2] == 1:
					button_instance.get_node("need2/Label").visible = false
			...
			get_node(categories_names[c]+"/VBoxContainer").add_child(button_instance)
		var end = load("res://gui/end.tscn")
		var ended = end.instance()
		get_node(categories_names[c]+"/VBoxContainer").add_child(ended)
